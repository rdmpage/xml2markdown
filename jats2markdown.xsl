...
<xsl:template match="text()">
  ...
  <xsl:template match="//article-meta">
    <!-- Citation line (journal / date / volume / pages) dropped — that metadata
         now lives in the YAML front matter emitted by jats2markdown.php. The H1
         title below is kept for readability. -->
    <xsl:text># </xsl:text>
 ...
 -->
  ...
  <!-- HTML-style inline elements inside cells (i/b/br/a). JATS uses
       italic/bold/break/ext-link, but XHTML-model tables may carry raw HTML
       inline markup; these keep italics/bold/links (e.g. taxon names) instead
       of letting the catch-all flatten them. -->
  ...
  <!-- Text inside a cell: collapse the source's pretty-print whitespace.
       ...
       which copies them VERBATIM (any newlines + indentation from pretty-printed
       source), breaking the table onto multiple lines. Mirror the edge-space
       handling of the default text() template so mixed cells like
